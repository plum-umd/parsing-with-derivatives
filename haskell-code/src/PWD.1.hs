...
data Parser t a where
  Parser :: (Ord t, Ord a) =>
    ...
    , parserMeta :: Meta a
    , pderive :: t -> Parser t a
    , compact :: Parser t a
    , computeMetaStep :: Parser t a
    , freeze :: Parser t a
    , parserChanged :: Bool 
    , parserFrozen :: Bool
    } -> Parser t a
...
a <|> b = fix $ \ self -> Parser (Alt a b) topMeta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) True False

(<~>) :: (Ord t, Ord a, Ord b) => Parser t a -> Parser t b -> Parser t (a, b)
a <~> b = fix $ \ self -> Parser (Con a b) topMeta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) True False

(==>) :: (Ord t, Ord a, Ord b) => Parser t a -> (a -> b) -> Parser t b
a ==> f = a |==> Set.map f

(|==>) :: (Ord t, Ord a, Ord b) => Parser t a -> (Set a -> Set b) -> Parser t b
a |==> f = fix $ \ self -> Parser (Red a f) topMeta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) True False

ter :: (Ord t) => t -> Parser t t
ter = terM . Set.singleton

terM :: (Ord t) => Set t -> Parser t t
terM tM = fix $ \ self -> Parser (Ter tM) meta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) False True
  where
...
epsM eM = fix $ \ self -> Parser (Eps eM) meta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) False True
  where
...
emp = fix $ \ self -> Parser Emp meta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) False True
  where
    meta = Meta { metaNullable = False
                , metaEmpty = True
                , metaFailAll = True
...
-- derive = strongEqMemo $ strongRefMemo . derive'

derive' :: t -> Parser t a -> Parser t a
derive' t p@(Parser _ _ _ _ _ _ _ _) = case parserRec p of
  (Alt a b) -> derive t a <|> derive t b
  (Con a b) -> derive t a <~> b <|> epsM (parseNull a) <~> derive t b
...
compact' :: Parser t a -> Parser t a
compact' p@(Parser _ _ _ _ _ _ _ _)
  | empty p = emp
  ...
  | otherwise = case parserRec p of
      ...
      (Con a b) 
        ...
        | otherwise -> compact a <~> compact b
      (Red (Parser (Red a' f) _ _ _ _ _ _ _) g) -> compact a' |==> g . f
      (Red a f) -> compact a |==> f
      (Ter _) -> p
...
computeMetaStep' :: Parser t a -> Parser t a
computeMetaStep' p@(Parser _ _ _ _ _ _ _ _)
  | parserFrozen p = p
  | otherwise = 
      let nextMeta = case parserRec p of
          ...
          nextRec = case parserRec p of
            ...
            (Eps _) -> error "impossible" -- case caught in frozen branch
            Emp -> error "impossible" -- case caught in frozen branch
      in fix $ \ self -> Parser nextRec nextMeta (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) (parserMeta p /= nextMeta) False
...
freeze' :: Parser t a -> Parser t a
freeze' p@(Parser _ _ _ _ _ _ _ _)
  | parserFrozen p = p
  | otherwise =
      let nextRec = case parserRec p of
            ...
            (Eps _) -> error "impossible" -- case caught in frozen branch
            Emp -> error "impossible" -- case caught in frozen branch
      in fix $ \ self -> Parser nextRec (parserMeta p) (strongEqMemo $ flip derive' self) (compact' self) (computeMetaStep' self) (freeze' self) (parserChanged p) True
